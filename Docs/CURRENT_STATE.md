...
- During Global Attack Cooldown the current baseline remains in Chase and pursues at full speed when outside the common `2.2m` outer attack range. Lower pursuit pressure, Wait/Strafe/Retreat/Approach choices and deliberate decision pacing remain deferred to the spacing/movement stage. Array order is only the current deterministic tie-breaker; per-attack cooldown readiness, Weight/random choice and a complete AI Agent are not implemented.
...
Preserve the runtime-verified two-attack range selector and add only per-attack cooldown duration plus per-enemy cooldown readiness as the next legality condition. Mutable deadlines must remain runtime state on the enemy, not inside shared `MeleeAttackData` assets. Do not add Weight/random selection, a complete AI Agent, Attack3, Strong Combo, spacing behaviors or the deferred Death precision tests in the same slice.
