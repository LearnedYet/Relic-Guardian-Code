...
Documentation updated: 2026-09-08. This is the sole maintained active Exact Next Step record. Actual code, saved Unity assets, current Editor state and Git status remain authoritative.
...
- Enemy remains the prototype chase/single timed attack flow; health depletion disables the enemy object.
...
- Enemy hits remain scheduled against the saved target without impact-time range/direction validation. A coarse Enemy state owner, reliable EnemyAttack cancellation boundary and real Death are not yet implemented.
...
Begin Enemy Combat Agent stage 2: establish a reliable EnemyAttack cancellation/cleanup boundary, then add the minimum coarse Enemy state owner and coordinate natural attack finish with that owner. Preserve EnemyAttackPhase as the attack executor's internal Ready/Startup/HitWindow/Recovery lifecycle.

The first single concept is the EnemyAttack execution cleanup contract: close future damage, remove any saved AttackThreatContext, clear target/timing/animation-start/telegraph state, and make repeated cancellation safe. Do not connect the newly imported Goblin Clips to Animator during this slice. Ordinary HitReaction, retained-corpse Death, Perfect Guard Stagger, multi-attack selection, Attack Hitstop, camera feedback and Strong Combo remain later stages. Explain responsibility and identifiers before learner code entry.
