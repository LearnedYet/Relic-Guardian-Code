...
| --- | --- | --- | --- | --- | --- |
| Relic Guardian | Active | `C:\Unity\Project\My project` | `Docs/PROJECT_PLAN.md`, `Docs/ROADMAP.md`, `Docs/LEARNING_PROGRESS.md` | Already started | Resume `EnemyMovement` by adding `rotationSpeed`, then implement movement-owned smooth facing before attaching or wiring chase |
...
1. Relic Guardian: bounded player startup lunge and Goblin Idle/Run animation switching are verified; `EnemyMovement` now owns a compiled horizontal `CharacterController.Move()` boundary. Next add only `rotationSpeed` and smooth movement-facing before attaching or wiring chase.
