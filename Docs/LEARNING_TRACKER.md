...
| --- | --- | --- | --- | --- | --- |
| Relic Guardian | Active | `C:\Unity\Project\My project` | `Docs/PROJECT_PLAN.md`, `Docs/ROADMAP.md`, `Docs/LEARNING_PROGRESS.md`, `Docs/COMBO_ATTACK_ARCHITECTURE.md` | Already started | Add only queued Attack input during the authored Combo Window; Attack2 and transition logic remain separate later steps |
...
1. Relic Guardian: Attack1 now uses the selected katana clip and combat Idle, one-entry `PlayerAttackData`, indexed reusable step initialization, and independent Combo Window events. Next add only `isAttackQueued` and input routing; Attack2, `ComboTransitionPoint`, and the late-recovery Attack1 Restart Window remain later steps.
