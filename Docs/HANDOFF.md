...
- Asset preparation has progressed through the verified two-hit checkpoint. `Attack_4Combo_1_Inplace` and `Attack_4Combo_2_Inplace` are connected; Attack3 and Attack4 remain unconnected.

### Runtime-Verified Two-Hit Combo Checkpoint
...
- No stale outgoing event reproduced during the verified two-hit path. Add explicit step/sequence identity protection before enabling the later Restart Window or extending the chain, because those flows increase overlap risk.
...
- The reusable Attack1-to-Attack2 path is implemented and runtime-verified. Resume with one concept: add explicit stale-animation-event identity protection before Restart Window or Attack3 work.
- The implemented two-hit architecture and confirmed later Restart Window are documented in `Docs/COMBO_ATTACK_ARCHITECTURE.md`. Keep that tracked file in every future sync to the code-focused GitHub repository and distinguish verified implementation from deferred target design.
